...
id_exception <- c("A10", "A11", "A12", "A13", "A22-m", "A34", "A35a", "A35b", "A37", "P15", "P16", "P17", "P18", "L03-a", "mesh1000", "mesh500", "P21", "N05")
...
```
    ...
    ## 
    ## $other
    ##  [1] "A10"      "A11"      "A12"      "A13"      "A22-m"    "A34"     
    ##  [7] "A35a"     "A35b"     "A37"      "A38"      "C28"      "L03-a"   
    ## [13] "mesh1000" "mesh500"  "N05"      "P15"      "P16"      "P17"     
    ## [19] "P18"      "P21"      "W09"     
    ## 
    ## $positional
...
d %>% 
  filter(id %in% A10_type) %>% 
  knitr::kable()
```

| id  | name                   | code | description                                    | type                                   |
|:----|:-----------------------|:-----|:-----------------------------------------------|:---------------------------------------|
| A10 | 都道府県コード         | NA   | 都道府県を一意に識別するために付された番号     | コードリスト「都道府県コード」         |
| A10 | 支庁区分コード         | NA   | 支庁区域を一意に識別するために付された番号     | コードリスト「支庁区分コード」         |
| A10 | 自然公園地域区分コード | NA   | 自然公園地域を一意に識別するために付された番号 | コードリスト「自然公園地域区分コード」 |
| A11 | 都道府県コード         | NA   | 都道府県を一意に識別するために付された番号     | コードリスト「都道府県コード」         |
| A11 | 支庁区分コード         | NA   | 支庁区域を一意に識別するために付された番号     | コードリスト「支庁区分コード」         |
| A11 | 自然保全地域区分コード | NA   | 自然保全地域を一意に識別するために付された番号 | コードリスト「自然保全地域区分コード」 |
| A12 | 都道府県コード         | NA   | 都道府県を一意に識別するために付された番号     | コードリスト「都道府県コード」         |
| A12 | 支庁区分コード         | NA   | 支庁区域を一意に識別するために付された番号     | コードリスト「支庁区分コード」         |
| A12 | 農業地区分コード       | NA   | 農業地域を一意に識別するために付された番号     | コードリスト「農業地域区分コード」     |
| A13 | 都道府県コード         | NA   | 都道府県を一意に識別するために付された番号     | コードリスト「都道府県コード」         |
| A13 | 支庁区コード           | NA   | 支庁区域を一意に識別するために付された番号     | コードリスト「支庁区分コード」         |
| A13 | 森林地域区分コード     | NA   | 森林地域を一意に識別するために付された番号     | コードリスト「森林地域区分コード」     |
...
purrr::walk(A10_type, function(id) {
  ...
  transmute(
    ...
    description = NA,
    type = NA,
    codelist = code %in% c("PREFEC_CD", "AREA_CD", "LAYER_NO")
  ) %>% 

...
tibble::enframe(errors) %>% 
  mutate(
    name = stringr::str_replace(basename(names(errors)), "(?:KsjTmplt-)(.*?)(?:-v.*)?(?:\\.html)", "\\1")
  ) %>% 
  knitr::kable()
```
...
| A28-11\_GML.zip                         | NA                                                                                                                                  |
| A29-19\_07\_GML.zip                     | Error in nchar(dsn): invalid multibyte string, element 1                                                                            |
...
| A37-15\_45\_GML.zip                     | Error in nchar(dsn): invalid multibyte string, element 1                                                                            |
...
| N09-12\_GML.zip                         | Error: Can’t rename columns that don’t exist.                                                                                       |
| x Column `N09_001` doesn’t exist.       |                                                                                                                                     |
